...
/**
 * Send an HTML warning email using Gmail
...
export async function sendEmailAlert(
  ...
  if (!process.env.GMAIL_USER || !process.env.GMAIL_PASS) {
    console.warn("⚠ GMAIL_USER or GMAIL_PASS not set!");
    return false;
  }
  ...
  const htmlContent = `
    ...
    ">
      ...
      <div style="
        ...
        line-height: 1.5;
      ">
        <strong>Important:</strong> If we do not receive a valid response
        via <b>email</b> or <b>rsync</b> within the next <b>1 hour</b>,
        your account may be <span style="color: #d9534f; font-weight: bold;">blocked</span> for security reasons.
      </div>

      <p style="font-size: 13px; color: #6c757d; text-align: center; margin-top: 20px;">
        This is an automated alert from <b>Geofence Systems</b>.  
  ...
  try {
    const info = await transporter.sendMail({
      from: `"Geofence Systems" <${process.env.GMAIL_USER}>`,
      to,
      subject,
      html: htmlContent,
    });
